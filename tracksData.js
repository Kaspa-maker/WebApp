import tracksModel from './api/tracks/tracksModel';
  ...
  export default async function loadTracks() {
    try {
      await tracksModel.deleteMany();
      await tracksModel.collection.insertMany(tracks);
      console.info(`${tracks.length} Tracks were successfully stored.`);
    } catch (err) {
